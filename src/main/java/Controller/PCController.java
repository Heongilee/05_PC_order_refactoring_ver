...
public class PCController implements Runnable {
   ...
   public static final C_login cl;
      ...
      GUI.addButtonActionListener(new ActionListener() {
         ...
         public void actionPerformed(ActionEvent e) {
            ...
            if (obj == GUI.LogOutbtn) { // 사용자 뷰에서 로그아웃 버튼을 눌렀을 경우
               ...
               outMsg.println(gson.toJson(
                     new Message(GUI.seat, GUI.id, "", GUI.msgInput.getText(), "sendtoadmin", "admins")));
               GUI.msgInput.setText("");
               ///////////////////////////////////
               // ↓↓↓↓ 상품 목록 버튼 ↓↓↓↓
               ///////////////////////////////////
               ...
               if (cu.Submit_Order()) {
                  outMsg.println(
                        gson.toJson(new Message(GUI.seat, GUI.id, "", "에서 결제하였습니다.", "orderSendServer", "")));
               } else {
               }
            }
         }
   ...
   public void returnImage() {
      try {
         ...
         if (nNode.getNodeType() == Node.ELEMENT_NODE) {
            ...
            GUI.weainfo[0] = new JLabel("지역: 서울특별시 광진구 화양동");
            GUI.weainfo[1] = new JLabel("현재 기온: " + getTagValue("temp", eElement) + " ºC");
            GUI.img = new ImageIcon("img/" + str + ".gif");
            GUI.weaimg = new JLabel(GUI.img);
            GUI.weainfo[2] = new JLabel("현재 상태: " + str);
         }
      } catch (Exception e) {
         e.printStackTrace();
      }
      for (int i = 0; i < 3; i++) {
         GUI.wea.add(GUI.weainfo[i]);
      }
      //패널에 부착
      GUI.weather.add(GUI.weaimg, BorderLayout.WEST);
      GUI.weather.add(GUI.wea, BorderLayout.CENTER);
      GUI.weather.setBackground(Color.WHITE);
      GUI.weather.setPreferredSize(new Dimension(250, 0));
   }
