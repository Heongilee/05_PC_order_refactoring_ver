...
public class C_ProdManager implements I_ProdManager{
	...
	@Override
	public void show() {
		if(PM.productComboBox.getSelectedIndex() == 0) {	//전체 상품 조회
			dao.SQL_SHOW(true, 0);
			PM.noticeTextLabel.setText("## 메시지 : 모든 상품 목록을 조회합니다...");
		}
		else {	//특정 상품 조회
			String id = (String) PM.productComboBox.getSelectedItem();
			dao.SQL_SHOW(false, Integer.valueOf(id));
			
			PM.noticeTextLabel.setText("## 메시지 : "+ id +"번 상품을 조회합니다...");
		}
		return;
	}
	...
	public void insertion() {
		if(PM.productComboBox.getSelectedIndex() == 0) { //PRODUCT 테이블에 튜플 삽입 메소드
		...
		for(int i=0;i<3;i++)
			PM.productTextFields[i].setText("");
	}
