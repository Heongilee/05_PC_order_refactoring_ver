...
public class C_UserView implements I_UserView {
	...
	public void Load_FoodCategory(int type) {
		...
		case 0:
			...
			break;
		case 1:
			menuList = dao.USERVIEW_FUNC1(GU.ca[1]);
			listModel = new DefaultListModel<Product_DTO>();
			...
			break;
		case 2:
			menuList = dao.USERVIEW_FUNC1(GU.ca[2]);
			listModel = new DefaultListModel<Product_DTO>();
			for (int i = 0; i < menuList.size(); i++) {
				listModel.addElement(menuList.get(i));
			}
			...
			break;
		case 3:
			menuList = dao.USERVIEW_FUNC1(GU.ca[3]);
			listModel = new DefaultListModel<Product_DTO>();
			for (int i = 0; i < menuList.size(); i++) {
				listModel.addElement(menuList.get(i));
			}
			...
			break;
		case 4:
			menuList = dao.USERVIEW_FUNC1(GU.ca[4]);
			listModel = new DefaultListModel<Product_DTO>();
			for (int i = 0; i < menuList.size(); i++) {
				listModel.addElement(menuList.get(i));
			}
	...
	public Boolean Submit_Order() {
		...
		int value = Integer.parseInt(GU.orderSumLabel.getText());// 합계를 가져오는 것
		...
		GU.ta1.setText("");
		GU.orderSum = 0;
		GU.orderSumLabel.setText(String.valueOf(GU.orderSum));
		return Order_Flag;
	}
}
